function state = metode3_v8_streaming_init(fs, N)
...
state.params.vocalBoostHighHz = 4000;
...
state.dyn.corrLR_ELR = 0;   
state.dyn.corrLR_EL2 = eps; 
state.dyn.corrLR_ER2 = eps; 

state.dyn.fluxMean = 0;
state.dyn.fluxVar  = 0;

state.dyn.bandPhasorBuf = zeros(numBands, state.params.timeSmoothFrames); 
state.dyn.bandPhasorBufCount = 0;

state.dyn.vlBuf = zeros(1, state.params.vlSmoothFrames); 
state.dyn.vlBufCount = 0;

state.dyn.gainBandPrev = ones(numBands,1); 

state.dyn.gainMedianBuf = ones(numBands, state.params.postMedianFrames); 

state.dyn.fracHighPLV_run = zeros(numBands,1); 
