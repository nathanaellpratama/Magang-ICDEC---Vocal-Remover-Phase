function [yL_frame, yR_frame, state, timing] = metode3_v9m_streaming_frame(L_hop, R_hop, state)
...
%% --- STFT FRAME TUNGGAL (pakai buffer N-sample yang sudah di-scroll) ---
...
%% --- PRE #3: PITCH/HARMONICITY (autocorrelation manual, code-gen compatible) ---
...
energy0 = sum(frame.^2) + eps; % normalisasi (setara opsi 'coeff' di lag=0)
...
%% --- DETEKSI RISIKO PER-BAND (EMA causal, ganti mean() global) ---
...
%% --- HARMONIC SUPPORT (1 frame, causal) ---
harmonicSupport = 0;
if f0 > 0
...
%% --- TRANSIENT PROTECTION (adaptive threshold, ganti prctile global) ---
...
fluxThresh = state.dyn.fluxMean + p.transientKStd * fluxStd; % approx dari prctile 85
isTransient = flux > fluxThresh;

gainBandRaw = gainBandRaw + (1-gainBandRaw) * (p.transientProtectAmount * isTransient);

%% --- ATTACK/RELEASE (causal, 1 langkah waktu) ---
